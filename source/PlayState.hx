...
class PlayState extends MusicBeatState
{
	...
	*/
    ...
    function set_hudSkin(value:String){
		...
		if (script == null)
		{
					var hxstuff = ["hscript", "hxs", "hxc", "hsc", "hxp", "hx"];
		for (i in 0...hxstuff.length)
		{
			var baseFile = 'hudskins/$value.${hxstuff[i]}';
	...
	override public function create()
	{
		...
		for (i in 0...hxstuff.length)
		{
				if(filesPushed.contains(file) || !file.endsWith('.${hxstuff[i]}'))
					return;
			}
...
/* 			for (note in notes) note.resizeByRatio(ratio);
	...
	*/
	...
	public function startCountdown():Void
	{
		...
		#end
		callOnScripts('onModchartCreated');
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (event in eventPushedMap.keys())
		{
			...
			for(file in ["events", #if PE_MOD_COMPATIBILITY "custom_events" #end]){
				...
				for (ext in ["hscript", "hxs", "hxc", "hsc", "hxp", "hx", #if LUA_ALLOWED "lua" #end])
				{
					if (doPush)
						break;
					var baseFile = '$baseScriptFile.$ext';
					...
					for (file in files)
					{
						...
						else #end if (ext == 'hscript'||ext=='hxc'||ext=='hsc'||ext=='hxp'||ext=='hxs'||ext=='hx')
						{
