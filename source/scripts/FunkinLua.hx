...
class FunkinLua extends FunkinScript
{
	...
	public function new(script:String, ?name:String, ?ignoreCreateCall:Bool=false) {
		...
		addCallback("numericForInterval", function(tag:String,start,end,interva,func){
			numericForInterval(start,end,interva,function(index){
				PlayState.instance.callOnScripts("onIntervalNumCount",[tag,index]);
			});
		});
